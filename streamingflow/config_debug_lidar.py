...
def set_cfg():
    ...
    _C.DATASET.CAMERA_MULTISWEEP = False
    _C.DATASET.CAMERA_SWEEP_STRIDE = 2
    _C.DATASET.CAMERA_MAX_SWEEPS = 6
    _C.DATASET.CAMERA_WINDOW_SEC = None
    _C.DATASET.CAMERA_SET_HI = ['CAM_FRONT']
    _C.DATASET.EVENTS_ROOT = ''
    ...
    _C.MODEL.MODALITY.USE_EVENT = False
    ...
    _C.MODEL.EVENT = CN()
    _C.MODEL.EVENT.BACKBONE = 'resnet50'
    _C.MODEL.EVENT.BINS = 10
    _C.MODEL.EVENT.IN_CHANNELS = 0  # if <= 0 -> use 2 * BINS
    _C.MODEL.EVENT.HIDDEN_DIM = 256
    _C.MODEL.EVENT.NUM_ENCODER_LAYERS = 1
    _C.MODEL.EVENT.NUM_HEADS = 8
    _C.MODEL.EVENT.FREEZE_BACKBONE = True
    _C.MODEL.EVENT.PRETRAINED = False
    _C.MODEL.EVENT.MULTISCALE_FUSION = 'sum'
    _C.MODEL.EVENT.FUSION_TYPE = 'independent'
    _C.MODEL.EVENT.BEV_FUSION = 'sum'
    _C.MODEL.EVENT.RESIDUAL_INIT = 0.0
    _C.MODEL.EVENT.NORMALIZE = False
...
def get_parser():
    ...
    parser.add_argument('--config-file', default='/home/user/sunsz/StreamingFlow/streamingflow/configs/dsec_lidar.yaml', metavar='FILE', help='path to config file')
    parser.add_argument(
