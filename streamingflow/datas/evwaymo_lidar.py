...
class DatasetEVWaymo_lidar(torch.utils.data.Dataset):
    ...
    def init_infos(self, sample_sequence_list):
        ...
        for k in range(len(sample_sequence_list)):
            ...
            if self.mode == 'train':
                info_path = os.path.join(self.dataroot, sequence_name, ('%s.pkl' % sequence_name)).replace('.pkl', '_fov_bbox_lidar_check.pkl')
            else:
                info_path = os.path.join(self.dataroot, sequence_name, ('%s.pkl' % sequence_name)).replace('.pkl', '_fov_bbox_lidar_check.pkl')    
    ...
    def get_infos_and_points(self, idx_list):
        ...
        for i in idx_list:
            ...
            #################### For disparity input #################
            lidar_path = os.path.join(self.dataroot, self.infos[i]['sequence_name']+'/lidar_fov/' + self.infos[i]['lidar_path'][-8:])
    ...
    def __getitem__(self, index):
        ...
        if 'annos' in current_info:
            ...
            gt_boxes_lidar = annos['gt_boxes_lidar']
            gt_len = len(annos['gt_boxes_lidar'])
            anno_names = annos['name']
            gt_obj_ids = annos['obj_ids']
            # print('abc', anno_names,gt_boxes_lidar)
...
if __name__ == '__main__':
    ...
    dsec = DatasetEVWaymo_lidar(data_cfg, cfg, is_train=True)
    print(len(dsec))
    dat = dsec[0]
    for n in range(3000):
        dat = dsec[n]
